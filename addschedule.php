<?php
	...
	//title
	echo "<h1 class='txt-white'>Add Bus Schedule</h1><br/>";
	...
	if(isset($_POST['submitted'])){
		...
		if ($validTicket && !$duplicateLoc && $validDepAndArr) {
			...
			//create an id for the schedule
			$ids = array();
			$sql2 = "SELECT * FROM schedule";
			$mydata2 = mysqli_query($con, $sql2);
			while ($row2 = mysqli_fetch_array($mydata2)) {
				array_push($ids, $row2['scheduleID']);
			}
			$newID = "s" . convertToTwoDigits(count($ids) + 1);
			
			$i = 2;
			
			while (in_array($newID, $ids)) {
				$newID = "s" . convertToTwoDigits(count($ids) + $i);
				$i++;
			}

			//sql query
			$sql = "INSERT INTO schedule (scheduleID, depLocation, arrLocation, depTime, arrTime, depDate, arrDate, ticketLeft, busID) 
			VALUES ('".$newID."', '".$depLoc."', '".$arrLoc."', '".$depTime."', '".$arrTime."', '".$depDate."', '".$arrDate."', '".$ticketAmt."', '".$bus."')";
			
			//if database being update successfully
			if(mysqli_query($con, $sql)) {
				echo "<p>Bus Schedule successfully added!</p>";
			}
			else {
				//if fail to update
				echo "<p style='color:red;'>Could not add the entry into database because:<br/>".mysqli_error($con).".The query was:". $sql.".</p>";
			}
			
			mysqli_close($con);
		}
		
		echo "<p><a href='addschedule.php'>Go Back</p>";
		...
		echo "<form class='text-left form-search' action='addschedule.php' method='post'>";
		...
		echo "<td><input type='text' name='ticket-amt' size='10' style='height: 50px; padding: 0 15px;' placeholder='No.' /></td>";
		echo "</tr>";
		echo "</table>";
		echo "</div>";
		...
		echo "<input type='text' name='dep-loc' size='22' style='height: 50px; padding: 0 15px;' placeholder='Enter Departure Location' /><br/><br/>";
		//departure time
		...
		displayDropdownOptions(0, 23, -1, 1);
		echo "</select>&nbsp;";
		echo "<select name='dep-min'>";	
		echo "<option value=-1>Minutes</option>";
		displayDropdownOptions(0, 55, -1, 5);
		echo "</select>";
		echo "</div></p><br/>";
		//departure date
		...
		echo "<input type='text' name='arr-loc' size='22' style='height: 50px; padding: 0 15px;' placeholder='Enter Arrival Location' /><br/><br/>";
		//arrival time
		...
		echo "<button type='submit' data-text='ADD SCHEDULE' class='btn btn-orange btn-fullwidth btn-winona btn-sm'><span>ADD SCHEDULE</span></button>";
		echo "<input type = 'hidden' name = 'submitted' value='true'/>";
		echo "</div>";
		echo "<div class='main-container'>";
		echo "<a href='import.php'>Click here to add multiple schedule at once!";
		echo "</div>";
		echo "</div>";
		echo "</form>";
		echo "</div>";
